  ...
  return (
    ...
    >
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex items-center justify-between w-full px-4 sm:px-6 py-2.5 text-sm font-medium text-foreground hover:bg-muted/40 transition-colors">
          <div className="flex items-center gap-2 overflow-hidden">
            <AlignLeft className="w-3.5 h-3.5 text-primary shrink-0" />
            <span className="font-semibold">On this page</span>
            {activeHeading && (
              <span className="text-xs text-muted-foreground truncate max-w-[220px] sm:max-w-[360px] font-normal">
                - {activeHeading.text}
              </span>
            )}
          </div>
              ...
              return (
                <a
                  ...
                  className={cn(
                    ...
                    heading.level === 3 ? "pl-3 text-xs" : "text-sm font-medium",
                    isActive
  ...
  return (
    <aside className={cn("hidden xl:block w-56 shrink-0 py-8 pl-6 sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto", className)}>
      <div className="space-y-4">
        <div className="flex items-center gap-1.5 text-sm font-semibold text-foreground tracking-tight">
